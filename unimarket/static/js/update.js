// 전체 체크 
function updateck() {
	if (!checkUserId(updateform.email.value)) {
		return false;
	}
	if (!checkPassword(updateform.email.value, updateform.passwd.value,
			updateform.passwd1.value)) {
		return false;
	}
	if (!checkName(updateform.nickname.value)) {
		return false;
	}
	if (!checkBdate(updateform.bDate.value)) {
		return false;
	}
	if (!checkLocation(updateform.location.value)) {
		return false;
	}
...
function setid(user_email) {
	opener.document.updateform.user_email.value = user_email
	opener.document.updateform.uniEmailChk.value = 1;
	window.close()
}
